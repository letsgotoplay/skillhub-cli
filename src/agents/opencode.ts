...
const configPaths: ConfigPath[] = [
  { type: 'project', path: '.opencode', filename: 'instructions.md' },
  { type: 'global', path: '.opencode', filename: 'instructions.md' },
];
...
function getConfigPath(options: InstallOptions): string {
  ...
  return path.join(projectRoot, '.opencode', 'instructions.md');
}
...
export const opencodeAgent: Agent = {
  ...
  async detectInstalled(): Promise<boolean> {
    return fs.pathExists(path.join(home, '.config', 'opencode'));
  },
