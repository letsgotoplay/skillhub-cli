...
const configPaths: ConfigPath[] = [
  { type: 'project', path: '.cline', filename: 'rules' },
  { type: 'global', path: '.cline', filename: 'rules' },
];

function getConfigPath(options: InstallOptions): string {
  if (options.global) {
    return path.join(os.homedir(), '.cline', 'rules');
  }
  const projectRoot = options.projectRoot || process.cwd();
  return path.join(projectRoot, '.cline', 'rules');
}
...
export const clineAgent: Agent = {
  ...
  async detectInstalled(): Promise<boolean> {
    return fs.pathExists(path.join(home, '.cline'));
  },
