...
const configPaths: ConfigPath[] = [
  { type: 'project', path: '.trae', filename: 'instructions.md' },
  { type: 'global', path: '.trae', filename: 'instructions.md' },
];

function getConfigPath(options: InstallOptions): string {
  if (options.global) {
    return path.join(os.homedir(), '.trae', 'instructions.md');
  }
  const projectRoot = options.projectRoot || process.cwd();
  return path.join(projectRoot, '.trae', 'instructions.md');
}
...
export const traeAgent: Agent = {
  ...
  async detectInstalled(): Promise<boolean> {
    return fs.pathExists(path.join(home, '.trae'));
  },
